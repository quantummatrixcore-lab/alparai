  ...
  try {
    ...
    if (!sourceResult.success) {
      return {
        ok: false,
        error:
          "Invalid source. Must be aiaaic_import, aiid_import, news_curated, or public_statements.",
      };
    }
    ...
    if (source === "public_statements") {
      const {
        rows,
        errors: parseErrors,
        total,
      } = (await import("@/lib/import/csv-parser")).parseStatementCSV(csvText);
      logger.info("Import Statement CSV parsed", {
        total,
      ...
      const importResult = await (
        await import("@/lib/import/incident-importer")
      ).importPublicStatements(rows, user.id);
      ...
      const {
        ...
        errors: parseErrors,
        total,
      } = (await import("@/lib/import/csv-parser")).parseIncidentCSV(csvText, source);
    ...
    console.error("[importIncidentsAction] Error:", err);
    throw err;
  }
}
