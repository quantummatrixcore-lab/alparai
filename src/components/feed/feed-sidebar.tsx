  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-6 lg:w-[320px]">
      ...
      <motion.div
        ...
        className="bg-bg-secondary/40 border-border-subtle/50 rounded-2xl border p-5"
      >
      ...
      {poll && (
        ...
        >
          ...
          <h3 className="text-fg-primary mt-3 mb-4 text-xs leading-normal font-bold">
            {locale === "tr" && poll.title_tr ? poll.title_tr : poll.title_en || poll.title}
          </h3>
...
function VoteBar({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="border-border-subtle relative h-3 flex-1 overflow-hidden rounded-sm border bg-white/5">
        <motion.div
          initial={{ width: 0 }}
          animate={{ width: `${pct}%` }}
        ...
        />
        {pct > 0 && (
          <span className="text-fg-secondary absolute top-1/2 right-1 -translate-y-1/2 text-[9px] font-bold">
            %{pct}
          </span>
        )}
      </div>
