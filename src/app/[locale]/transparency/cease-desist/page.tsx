    ...
    <Container className="py-12">
      ...
      <div className="to-bg-secondary/40 relative mb-12 overflow-hidden rounded-3xl border border-red-500/30 bg-gradient-to-b from-red-950/40 via-red-900/10 p-8 shadow-[0_0_50px_rgba(239,68,68,0.15)] backdrop-blur-2xl md:p-12">
        ...
        <div className="relative z-10 max-w-3xl space-y-4">
          ...
          <div className="text-fg-muted border-danger-500/20 flex flex-wrap gap-4 border-t pt-4 font-mono text-xs">
            <div className="flex items-center gap-1.5">
              <Eye className="h-4 w-4 text-red-400" />
              <span>
                {t("publishRate")}: {t("publishRate_value")}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <Lock className="h-4 w-4 text-red-400" />
              <span>{t("cease_desist_immunity")}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Scale className="h-4 w-4 text-red-400" />
              <span>{t("cease_desist_exemption")}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {logs.length === 0 ? (
        <div className="to-bg-secondary/30 border-danger-500/20 rounded-2xl border bg-gradient-to-br from-red-950/20 p-12 text-center backdrop-blur-md">
        ...
        <div className="space-y-6">
          {logs.map((log) => {
            const providerName = log.ai_providers?.name ?? t("cease_desist_unknown_provider");
            ...
            return (
              ...
              >
                ...
                {/* Card Top Metadata */}
                <div className="border-danger-500/20 flex flex-wrap items-center justify-between gap-4 border-b pb-4">
