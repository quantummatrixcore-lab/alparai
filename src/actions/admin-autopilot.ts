...
export async function getAdminAutopilotSnapshot(limit = 100): Promise<AdminAutopilotResult> {
  try {
    ...
    const availableRaw = q.available;
    const isAvailable: boolean =
      typeof availableRaw === "function"
        ? Boolean(await (availableRaw as () => number | Promise<number>)())
    ...
    return {
      ...
      snapshot: {
        ...
        breakers,
        policies,
        queue: { available: isAvailable, size },
        workerConfigs,
        globalKillSwitch,
        cronLogs,
        activeEngines,
    ...
    console.error("[getAdminAutopilotSnapshot] Error:", err);
    throw err;
  }
}
...
export async function triggerAutopilotWorkerTick(): Promise<{
  ...
  try {
    ...
    console.error("[triggerAutopilotWorkerTick] Error:", err);
    throw err;
  }
}
...
export async function toggleAutopilotWorker(
  ...
  try {
    ...
    console.error("[toggleAutopilotWorker] Error:", err);
    throw err;
  }
}
