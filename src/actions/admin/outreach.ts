...
export async function getOutreachQueue() {
  try {
    ...
    if (error) {
      console.error("[getOutreachQueue] DB error:", error);
...
export async function processOutreachQueueAction() {
  try {
    ...
    console.error("[processOutreachQueueAction] Error:", err);
    throw err;
  }
}
...
export async function getOutreachStats() {
  try {
    const supabase = createAdminClient();
    const { data, error } = await supabase.from("outreach_queue").select("status");

    if (error || !data) {
      return { pending: 0, approved: 0, sent: 0, failed: 0, limit: 50 };
    }
    ...
    return { pending, approved, sent, failed, limit: 50 };
  } catch {
    return { pending: 0, approved: 0, sent: 0, failed: 0, limit: 50 };
  }
}
