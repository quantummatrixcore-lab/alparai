...
import { Shield, ShieldCheck, LogIn, Lock } from "lucide-react";
...
export default async function SubmitPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <Container size="narrow" className="py-12">
      ...
      <Link
        href="/submit/whistleblower"
        className="mb-6 block rounded-lg border border-purple-500/30 bg-purple-500/10 p-4 text-xs transition-colors hover:bg-purple-500/20"
      >
        <div className="mb-1 flex items-center gap-2 font-semibold text-purple-300">
          <Shield className="h-4 w-4 text-purple-400" />
          <span className="underline decoration-purple-400/50 underline-offset-2">
            {t("whistleblower_title")}
          </span>
        </div>
      ...
      {!isLoggedIn ? (
        <Card className="border-brand-500/20 bg-brand-500/5">
          <CardContent className="flex flex-col items-center py-16 text-center">
            ...
            <h2 className="text-fg-primary text-2xl mb-3 font-bold">
            ...
            <div className="space-y-3">
              ...
              <div className="pt-4 border-t border-border-subtle/50">
                <Link href="/submit/whistleblower">
                  <Button variant="outline" size="md" leftIcon={<Lock className="h-4 w-4 text-purple-400" />}>
                    {t("anonymous_whistleblower_btn")}
                  </Button>
                </Link>
              </div>
            </div>
