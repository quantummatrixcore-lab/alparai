...
import { ProviderCombobox, type ComboboxOption } from "@/components/models/provider-combobox";
import { ModelAutocomplete, type ModelOption } from "@/components/models/model-autocomplete";
...
export function IncidentForm({
    ...
    if (processingStage) {
      ...
      return (
        <div className="mx-auto max-w-md space-y-8 px-4 py-16 text-center">
          ...
          {/* Progress Card */}
          <div className="border-brand-500/20 bg-bg-primary/40 rounded-2xl border p-6 shadow-2xl backdrop-blur-md">
            <div className="space-y-6 text-left">
              {stages.map((stage, idx) => {
                ...
                if (isPast) {
                  icon = (
                    <div className="bg-emerald-accent/20 flex h-6 w-6 items-center justify-center rounded-full">
                      <CheckCircle2 className="text-emerald-accent h-4 w-4" />
                    </div>
                  );
                } else if (isCurrent) {
                  icon = (
                    <div className="flex h-6 w-6 items-center justify-center">
                      <div className="border-emerald-accent h-4 w-4 animate-spin rounded-full border-2 border-t-transparent" />
                    </div>
                  );
                } else {
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-medium ${
                        isCurrent
                          ? "text-emerald-accent shadow-sm drop-shadow-[0_0_8px_rgba(0,255,136,0.3)]"
                          : "text-fg-primary"
                      }`}
                    >
  ...
  const canSubmit = allConsents && selectedProvider && description.trim().length >= 10;
  ...
  const handleClientAction = async (formData: FormData) => {
    ...
    const { maskPII } = await import("@/lib/pii/guardian");
    ...
    const input = {
      title: maskPII(effectiveTitle).masked,
      description: maskPII(rawDescription).masked,
  ...
  return (
    <form action={handleClientAction} className="space-y-6">
      ...
      <Input
        name="title"
        label={`${t("title_placeholder")} (Optional)`}
        maxLength={200}
        value={title}
            ...
            {severityOptions.map((opt) => {
              ...
              const borderColors = {
                low: isActive
                  ...
                  : "border-border-subtle bg-bg-secondary/20 hover:border-border-strong text-fg-secondary",
                medium: isActive
